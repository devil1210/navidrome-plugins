...
def _apply_romanization(api, track, metadata, file=None):
    cfg = api.plugin_config if (api and hasattr(api, "plugin_config")) else config.setting
...
class AutoRomanizerOptionsPage(OptionsPage):
    ...
    def load(self):
        mode = self.api.plugin_config[TITLE_MODE_OPTION] if TITLE_MODE_OPTION in self.api.plugin_config else DEFAULT_MODE
        idx = self.combo_mode.findData(mode)
        if idx >= 0:
            self.combo_mode.setCurrentIndex(idx)

    def save(self):
        self.api.plugin_config[TITLE_MODE_OPTION] = self.combo_mode.itemData(self.combo_mode.currentIndex())
